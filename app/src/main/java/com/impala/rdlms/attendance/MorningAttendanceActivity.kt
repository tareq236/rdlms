...
class MorningAttendanceActivity : AppCompatActivity() {
    ...
    private fun captureImage() {
        ...
        if (intent.resolveActivity(packageManager) != null) {
            ...
            val imageUri =
                FileProvider.getUriForFile(this, "com.impala.rdlms.provider", imageFile)
                    ...
                    getCurrentLocation(object : OnSuccessListener<Location> {
                        override fun onSuccess(location: Location?) {
                            if (location != null) {
                                ...
                                val imagePart = MultipartBody.Part.createFormData("start_image", compressedImageFile.name, imageRequestBody)
                                ...
                                val srIdRequestBody = RequestBody.create(MediaType.parse("text/plain"), sharedPreferences.getString("sap_id", ""))

                                val apiService = ApiService.CreateApi1()
                                val call = apiService.saveMorningAttendanceWithImage(srIdRequestBody, latitudeRequestBody, longitudeRequestBody, imagePart)
                                        ...
                                        if (response.isSuccessful) {
                                            ...
                                            if (apiResponse != null) {
                                                if(apiResponse.success){
                                                    ...
                                                    showDialogBox(SweetAlertDialog.SUCCESS_TYPE, "SUCCESS", "Morning Attendance Submit Successfully")
                                                }else{
                                                    dismissLoadingDialog()
                                                    showDialogBox(SweetAlertDialog.WARNING_TYPE, "Waring", apiResponse.message)
                                                }
                                            }else{
                                                dismissLoadingDialog()
        ...
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
            imageFilePath?.let { path ->
                val bitmap = MediaStore.Images.Media.getBitmap(
                    contentResolver,
                    FileProvider.getUriForFile(this, "com.impala.rdlms.provider", File(path))
                )
    ...
    private fun showDialogBox(type: Int, title: String, message: String, callback: (() -> Unit)? = null) {
        val sweetAlertDialog = SweetAlertDialog(this, type)
            ...
            .setConfirmClickListener {
                startActivity(Intent(this@MorningAttendanceActivity, MainActivity::class.java))
                finish()
            }
